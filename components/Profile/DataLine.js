...
const DataLine = (props) => {
  ...
  const genderRate = (() => {
    ...
    if (props.tag == 'Gender') {
      if (props.data == -1) { return <Text style={styles.boxAboutData}>Genderless</Text> }
      let female = props.data / 8 * 100;
      let male = 100 - female;
      return (<View style={[styles.boxAboutData, styles.genderContainer]}><Text style={styles.male}>♂ {male}% / </Text><Text style={styles.female}>♀ {female}%</Text></View>)
    }
  })
  ...
  const weakness = (() => {
    ...
    if (props.data.length == 1) {
      for (let defences in defencesData[typeA]) {
        ...
        if (defencesData[typeA][defences] >= 1) {
          weaknesses.push(<TypeDefencesIcon key={defences} type={defences} effectiveness={defencesData[typeA][defences]} />)
        }
      }
    }
    if (props.data.length == 2) {
      ...
      for (let defences in defencesData[typeA]) {
        ...
        if (defencesData[typeA][defences] == null || defencesData[typeB][defences] == null) {
          continue;
        }
        if (defencesData[typeA][defences]+defencesData[typeB][defences] >= 1) {
          weaknesses.push(<TypeDefencesIcon key={defences} type={defences} effectiveness={defencesData[typeA][defences] + defencesData[typeB][defences]} />)
        }
      }
    }
    if (weaknesses.length == 0) return (<Text style={styles.boxAboutData}>No weaknesses!</Text>);
    return weaknesses;
  })
