...
const GenerationTile = (props) => {
    const [isActive, setIsActive] = useState(props.tilesSelected[props.generationId-1]);
    ...
    return (
        <TouchableOpacity onPress={() => handleClick()}>
            <View style={[styles.container, {backgroundColor: isActive ?  globalStyles.typepsychic : globalStyles.backgrounddefaultinput}]}>
                <Image style={styles.pokeball} source={Pokeball} />
                <Image style={styles.dots} source={Dots} />
                <View style={styles.imgContainer}>
                    <Image source={images[props.generationId]}
                        style={{
                            width: '100%',
                            height: '100%',
                        }} />
                </View>
                <Text style={[styles.title, {color: isActive ? globalStyles.textwhite : globalStyles.textgrey}]}>Generation {romanize(props.generationId)}</Text>
            </View>
        </TouchableOpacity>
    )
}
